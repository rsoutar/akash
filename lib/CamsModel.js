...
// ---------------------------------------------------------------------------
// CAMS rendering scale
// ---------------------------------------------------------------------------

// The scale the CAMS overlay is actually drawn with, read off the GetLegend
// image of each layer's default style (styles="" — the same request the map
// makes). Each table is a list of { color, upTo } rungs, low→high: a reading
// below `upTo` paints `color` on the map, so the legend rungs and the pixels
// the map shows are the same table. Units: µg/m³ for the gases and particulates,
// AOD (dimensionless) for the aerosol layers.
//
// Band colours repeat across species because CAMS renders the pollutant
// concentrations with one shared multihue palette — the legends differ only in
// the numeric ticks. The tables here are pinned by test/cams-model.test.js
// against values read from the served legends; if CAMS restyles a layer these
// drift and the legend must be re-read from the new image.
var CAMS_SCALES = {
  "pm2p5": { unit: "µg/m³", ticks: [2, 5, 10, 20, 30, 40, 50, 75, 100, 150, 200, 500], colors: [
    "#C6E9F3", "#B3DFEB", "#A0D5E3", "#BFDFCD", "#EBEEB3", "#FAE88E", "#F5D363", "#EDB43D",
    "#E18621", "#D05D0C", "#AA4211", "#852716"
  ] },
  "pm10": { unit: "µg/m³", ticks: [2, 5, 10, 20, 30, 40, 50, 75, 100, 150, 200, 500], colors: [
    "#C6E9F3", "#B3DFEB", "#A0D5E3", "#BFDFCD", "#EBEEB3", "#FAE88E", "#F5D363", "#EDB43D",
    "#E18621", "#D05D0C", "#AA4211", "#852716"
  ] },
  "o3": { unit: "µg/m³", ticks: [0, 20, 40, 60, 80, 100, 120, 140, 160, 180, 200, 240, 500], colors: [
    "#C6E9F3", "#B3DFEB", "#A0D5E3", "#BFDFCD", "#EBEEB3", "#FAE88E", "#F5D363", "#EDB43D",
    "#E18621", "#D05D0C", "#AA4211", "#852716", "#3A1C14"
  ] },
  "no2": { unit: "µg/m³", ticks: [0, 2, 5, 10, 20, 30, 40, 50, 75, 100, 150, 200, 300], colors: [
    "#C6E9F3", "#B3DFEB", "#A0D5E3", "#BFDFCD", "#EBEEB3", "#FAE88E", "#F5D363", "#EDB43D",
    "#E18621", "#D05D0C", "#AA4211", "#852716", "#40140E"
  ] },
  "so2": { unit: "µg/m³", ticks: [0, 2, 5, 10, 20, 30, 40, 50, 75, 100, 150, 200, 800], colors: [
    "#C6E9F3", "#B3DFEB", "#A0D5E3", "#BFDFCD", "#EBEEB3", "#FAE88E", "#F5D363", "#EDB43D",
    "#E18621", "#D05D0C", "#AA4211", "#852716", "#220A06"
  ] },
  "co": { unit: "µg/m³", ticks: [0, 50, 100, 150, 200, 250, 300, 350, 400, 500, 700, 1000, 2000], colors: [
    "#C6E9F3", "#B3DFEB", "#A0D5E3", "#BFDFCD", "#EBEEB3", "#FAE88E", "#F5D363", "#EDB43D",
    "#E18621", "#D05D0C", "#AA4211", "#852716", "#2B0F08"
  ] },
  "aod550": { unit: "AOD", ticks: [0.15, 0.2, 0.35, 0.5, 0.8, 1, 3], colors: [
    "#004CFF", "#29FFCE", "#7DFF7A", "#CEFF29", "#FFC500", "#FF6800", "#F20700", "#800000"
  ] }
}

// AOD's own legend prints 0.15 … 3 as its breaks; the first and last colours
// cover below 0.15 and above 3. Encode the breaks the way the other tables do:
// a rung is "below upTo".

// The legend rows for a species: one per CAMS band, in display order, each
// with its colour and the value the band tops at. Unknown species answer empty
// — the legend then shows no bar rather than a wrong one.
function camsLegendRows(species) {
  var scale = CAMS_SCALES[species]
  if (!scale) return []
  var rows = []
  for (var i = 0; i < scale.ticks.length; i++) {
    rows.push({ color: scale.colors[i], upTo: scale.ticks[i], unit: scale.unit })
  }
  return rows
}

// The unit a species' CAMS scale is read in, for the legend's caption.
function camsScaleUnit(species) {
  var scale = CAMS_SCALES[species]
  return scale ? scale.unit : ""
}
...
// The two words at the ends of a category's ramp. `category` is the chip id,
// which picks its own ends — air quality reads "Cleaner / More polluted",
// Allergens "Less pollen / More pollen", and so on. A category with no declared
// ends (the advanced tail) falls back to the air-quality pair; a ramp without
// its ends would be a scale without its names.
