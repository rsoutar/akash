...
// written by an older plugin answers null, and the caller decodes the shipped
// file exactly as it did before; a corrupt cache costs one decode, not a map.
// The cache repeats the basemap's format version and carries one of its own,
// so a change to either makes the old file unreadable on purpose.
//
// tools/build-basemap.py regenerates data/basemap.bin, so CACHE_VERSION must
// be bumped in the same change: the format version only moves when the layout
// of the file moves, and a rebuild that keeps the layout would otherwise be
// shadowed by a cache of the old ground.
...
function cacheSize(map) {
  var size = 4 + 1 + 1 + varintSize(map.quantum) + varintSize(map.order.length)
...
function deserializeCache(buffer) {
  ...
  try {
    ...
    if (reader.u8() !== FORMAT_VERSION) throw new Error("cache: unknown basemap version")
