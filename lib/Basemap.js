...
.import "BasemapDigest.js" as BasemapDigest

var MAGIC = "OWRB"
var FORMAT_VERSION = 2

// Feature kinds, as written by tools/build-basemap.py. Lines and polygons
// decode through the same polygon path; only places differ.
var POINTS = 2
...
// Decoding the shipped file costs the shell most of a second once per session,
// and the session starts over at every shell restart, so the geometry is also
// written out decoded. The cache is a second representation of the same data:
// typed arrays laid end to end in one buffer, read back as views into it, with
// no varint per point and no per-point objects at all.
//
// It is a cache and never an authority. Anything unreadable, truncated, or
// stamped with another basemap answers null, and the caller decodes the
// shipped file exactly as it did before; a corrupt cache costs one decode, not
// a map.
//
// What makes a cache current is the digest of data/basemap.bin, generated by
// tools/build-basemap.py into BasemapDigest.js and stamped into every cache.
// Rebuilding the ground changes the digest, so every cached decode is refused
// by construction: there is no version to remember to bump and no way for a
// cache of the old ground to shadow a new one. CACHE_VERSION covers only the
// cache's own layout, and moves when the fields below do.
var CACHE_MAGIC = "AKBC"
var CACHE_VERSION = 2

function isPoints(layer) {
  return layer.kind === POINTS
}

// Bytes one varint of this value takes, matching Reader.varint's arithmetic.
...
function signedVarintSize(value) {
  return varintSize(value < 0 ? (-value * 2) - 1 : value * 2)
}

// A place name as UTF-8 bytes. Reader.string() decodes these; the two have to
// agree, and the tests hold them against each other.
...
// Int32Array views must start on a four-byte boundary, so every block of raw
// coordinates is padded up to one. Both sides of the round trip use this, so
// they compute the same offsets without any padding byte being read.
function align4(offset) {
  return (offset + 3) & ~3
}

// Bytes the decoded arrays of one geometry layer take, in the order they are
// written. Ring bounds are one longer than the ring count, feature bounds four
// per feature, and chunk bounds four per run. Both sides of the round trip
// derive their offsets from this, so they cannot disagree about the layout.
function geometryBytes(pointTotal, ringTotal, featureCount, chunkCount) {
  return (pointTotal * 2 + (ringTotal + 1) + (featureCount + 1)
    + featureCount * 4 + (ringTotal + 1) + chunkCount * 4) * 4
}

// The exact size serializeCache will write, so it can allocate once instead of
// growing a buffer and copying several megabytes repeatedly.
function cacheSize(map) {
  var digest = utf8Bytes(BasemapDigest.BASEMAP_SHA256)
  var size = 4 + 1 + varintSize(digest.length) + digest.length
    + varintSize(map.quantum) + varintSize(map.order.length)
  for (var i = 0; i < map.order.length; i++) {
    var layer = map.layers[map.order[i]]
    var name = utf8Bytes(layer.name)
    size += varintSize(name.length) + name.length + 3 + varintSize(layer.featureCount)
    if (isPoints(layer)) {
      size += varintSize(layer.places.length)
      for (var p = 0; p < layer.places.length; p++) {
        var place = layer.places[p]
        var placeName = utf8Bytes(place.name)
        // x and y as signed varints, the zoom byte, then the name's length and
        // bytes. The length is a varint too, so it is not always one byte.
        size += signedVarintSize(place.x) + signedVarintSize(place.y) + 1
          + varintSize(placeName.length) + placeName.length
      }
    } else {
      // The array counts, then the arrays themselves, the first of which is
      // padded up to a four-byte boundary.
      size += varintSize(layer.coordinates.length / 2)
        + varintSize(layer.ringStart.length - 1)
        + varintSize(layer.chunkCount)
      size = align4(size)
      size += geometryBytes(layer.coordinates.length / 2, layer.ringStart.length - 1,
        layer.featureCount, layer.chunkCount)
    }
  }
  return align4(size)
}

// The decoded map as one ArrayBuffer, to be handed to FileView.setData.
function serializeCache(map) {
  var buffer = new ArrayBuffer(cacheSize(map))
  ...
  function signed(value) {
    varint(value < 0 ? (-value * 2) - 1 : value * 2)
  }

  function string(text) {
    var encoded = utf8Bytes(text)
    varint(encoded.length)
    for (var i = 0; i < encoded.length; i++) bytes[at++] = encoded[i]
  }

  function int32Array(array) {
    at = align4(at)
    new Int32Array(buffer, at, array.length).set(array)
    at += array.length * 4
  }

  for (var m = 0; m < 4; m++) u8(CACHE_MAGIC.charCodeAt(m))
  u8(CACHE_VERSION)
  string(BasemapDigest.BASEMAP_SHA256)
  varint(map.quantum)
  varint(map.order.length)
  ...
  for (var i = 0; i < map.order.length; i++) {
    ...
    u8(layer.minZoom)
    u8(layer.maxZoom)
    varint(layer.featureCount)

    if (isPoints(layer)) {
      varint(layer.places.length)
      for (var p = 0; p < layer.places.length; p++) {
        var place = layer.places[p]
        signed(place.x)
        signed(place.y)
    ...
    varint(layer.coordinates.length / 2)
    varint(layer.ringStart.length - 1)
    varint(layer.chunkCount)
    int32Array(layer.coordinates)
    int32Array(layer.ringStart)
    int32Array(layer.featureRing)
    int32Array(layer.bounds)
    int32Array(layer.ringChunk)
    int32Array(layer.chunkBounds)
  }

  return buffer
}

// The map a cache file holds, or null for anything that is not one this
// version of the plugin wrote. Longitude and latitude come back from x and y,
// as they do in the decoder; only the name and the threshold are stored.
function deserializeCache(buffer) {
  ...
  try {
    ...
    if (magic !== CACHE_MAGIC) throw new Error("cache: not a decoded basemap")
    if (reader.u8() !== CACHE_VERSION) throw new Error("cache: unknown cache version")
    if (reader.string() !== BasemapDigest.BASEMAP_SHA256) {
      throw new Error("cache: stamped with another basemap")
    }

    var quantum = reader.varint()
    if (!(quantum > 0)) throw new Error("cache: quantum must be positive")
    ...
    for (var i = 0; i < layerCount; i++) {
      ...
      if (isPoints(layer)) {
        ...
        var pointTotal = reader.varint()
        var ringTotal = reader.varint()
        var chunkCount = reader.varint()

        var base = align4(bytes.byteOffset + reader.at) - bytes.byteOffset
        reader.at = base
        var end = base + geometryBytes(pointTotal, ringTotal, layer.featureCount, chunkCount)
        if (end > reader.end) throw new Error("cache: geometry runs past end of data")

        layer.coordinates = new Int32Array(buffer, base, pointTotal * 2)
        base += pointTotal * 2 * 4
        layer.ringStart = new Int32Array(buffer, base, ringTotal + 1)
        base += (ringTotal + 1) * 4
        layer.featureRing = new Int32Array(buffer, base, layer.featureCount + 1)
        base += (layer.featureCount + 1) * 4
        layer.bounds = new Int32Array(buffer, base, layer.featureCount * 4)
        base += layer.featureCount * 4 * 4
        layer.ringChunk = new Int32Array(buffer, base, ringTotal + 1)
        base += (ringTotal + 1) * 4
        layer.chunkBounds = new Int32Array(buffer, base, chunkCount * 4)
...
// coastline. Antarctica's closing edges have to exist for the fill to be a
// closed shape, and drawing them as coastline puts a stroke down the middle of
// the map for anyone who pans to the antimeridian at the southern limit.
//
