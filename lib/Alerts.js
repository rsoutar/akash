...
// Whether a reading is worth telling the user about, given what they were last
// told. Returns the decision and the latch value to keep, so the caller stores
// one thing and cannot get the two out of step.
//
...
// escalates. It is cleared only once the reading drops back under the
// threshold, so a value that flickers around the boundary cannot re-notify.
// A missing reading (-1) holds the latch rather than re-arming: a broken probe
// is not an improvement in the air.
//
// The storm alert and the air-quality alert are the same rule over different
// ladders — `decide` is the only latch, and they pass their own `current`
// and `threshold` into it.
function decide(current, threshold, notified, enabled) {
  if (!enabled) return { notify: false, notified: 0 }
  if (current < 0) return { notify: false, notified: notified }
  if (current < threshold) return { notify: false, notified: 0 }
  if (current <= notified) return { notify: false, notified: notified }
  return { notify: true, notified: current }
}
...
// and their names come from CamsModel.BAND_NAMES (the same six-name ladder the
// panel's legend and the settings tests read). The threshold is a band rather
// than a concentration because the cut-offs are per species — "Poor" means
// 25 µg/m³ of PM2.5 and 90 of NO2, and one number cannot say both.
//
// The latch itself is the storm alert's `decide` above, called with
// `currentBand`, `thresholdBand` and `notifiedBand`. A missing reading (-1)
// holds the latch rather than re-arming: a broken probe is not an improvement
// in the air.
