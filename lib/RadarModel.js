...
// The map's legend row: one named rung of the intensity scale. The name comes
// from the band table above, the position from the band's place on the 0-255
// scale the tiles are decoded from, so the label sits where the colour change
// it names falls on the ramp the tile paints. The two ends are not rungs of
// their own: "clear" paints nothing at all, and "severe" is simply "everything
// from heavy upward".
function radarLegendBands() {
  return [
    { name: intensityLabel(INTENSITY_LIGHT), value: INTENSITY_LIGHT },
    { name: intensityLabel(INTENSITY_MODERATE), value: INTENSITY_MODERATE },
    { name: intensityLabel(INTENSITY_HEAVY), value: INTENSITY_HEAVY }
  ]
}

// How far along the 0-255 scale a band's value sits, as a fraction a rendered
// ramp can anchor a label at.
function radarLegendFraction(value) {
  var maximum = 255
  var clipped = Math.max(0, Math.min(maximum, Number(value)))
...
// (measured 2026-09-09 at z7, the deepest zoom RainViewer serves). So there is
// one honest ramp to show, with exactly the steps the tile quantises to —
// faintest at one end, strongest at the other.
//
...
var RADAR_GRADIENT_STOPS = [
  "#a80000", "#004768", "#006295", "#0070a3", "#007fb4", "#0088bf",
  "#857d6a", "#928871", "#9e9375", "#aa9e79", "#b6a97e", "#c2b482",
  "#cec087", "#d2c48b", "#d6c88f", "#dacc93", "#ded097", "#ffe000", "#ffee00"
]
