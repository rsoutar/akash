...
// map hostage to it.
//
// It stops at z11 because that is deep enough to read the road network between
// towns. The basemap is Natural Earth at 1:10 million with its 1:10m roads on
// top; past z11 the coastline would be magnified beyond the accuracy it claims,
// over radar that was already being upscaled four levels earlier. A zoom level
// where neither layer has anything left to say is a zoom level that only looks
// like detail.
...
// Exact GPS coordinates, for people whose city centre is kilometres from
// where they live. A geocoded city is a good-enough point; a rain forecast
// can be a different answer a few streets over. Both enter the same shared
// weather.json through the same omarchy-weather-location call, so a typed
// coordinate is not a second location, it is a sharper version of the same
// one — the radar centre, the forecast samples, the alert radius and the
// air-quality probe all simply follow the file.
//
// The text is checked against the shape omarchy-weather-location itself
// accepts, so a value this function passes is a value the CLI stores: an
// optional sign, digits, an optional fractional part, nothing else. Two
// separate fields mean nothing here but a plain pair of numbers — a comma in
// either is a mistake, not a second separator to be split off.
var COORDINATE_PATTERN = /^-?\d+(\.\d+)?$/

// Trimmed and range-checked GPS input → a real point, or null. Null covers
// every way the fields can be unusable — empty, not a number, unbalanced
// signs or exponents — so the caller shows one error rather than five.
//
// Longitude up to ±360 is accepted and wrapped onto the globe: some GPS
// hardware reports east of Greenwich as 0–360 instead of −180..180, and a
// reading of 180.5°E is a real place (−179.5°), so rejecting it would throw
// away a genuine position. A value past ±360 is a typo, not a wrap, and still
// fails. Latitude has no second convention to honour — the world does not
// wrap north–south — and stays strict at ±90.
function parseCoordinates(latText, lonText) {
  var lat = String(latText || "").trim()
  var lon = String(lonText || "").trim()
  if (!COORDINATE_PATTERN.test(lat) || !COORDINATE_PATTERN.test(lon)) return null

  var latitude = parseFloat(lat)
  var longitude = parseFloat(lon)
  if (!isFinite(latitude) || !isFinite(longitude)) return null
  if (latitude < -90 || latitude > 90) return null
  if (longitude < -360 || longitude > 360) return null
  if (longitude < -180 || longitude > 180) longitude = TileMath.wrapLongitude(longitude)
...
// stops painting a gradient inside it. The names describe the colour, not a
// dBZ figure: the blue family opens at the extreme end of the ramp, and the
// legend says so with its own three words.
//
...
// finally red as the core intensifies.
//
...
// spacing between opaque bins is an assumption.
//
